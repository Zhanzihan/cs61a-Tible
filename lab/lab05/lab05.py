def couple(s, t):
    ...
    "*** YOUR CODE HERE ***"
    length = len(s)
    return [[s[i], t[i]] for i in range(length)]
...
def closer_city(lat, lon, city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    _ = make_city('_', lat, lon)
    if distance(_, city_a) < distance(_, city_b):
...
def add_trees(t1, t2):
    ...
    "*** YOUR CODE HERE ***"
    if is_leaf(t1):
        return tree(label(t1)+label(t2), [x for x in branches(t2)])
    if is_leaf(t2):
        return tree(label(t1)+label(t2), [x for x in branches(t1)])
    if len(branches(t1)) < len(branches(t2)):
        return tree(label(t1)+label(t2), [add_trees(x, y) for (x, y) in zip(branches(t1), branches(t2))] + [x for x in branches(t2)[len(branches(t1)):]])
    else:
        return tree(label(t1) + label(t2), [add_trees(x, y) for (x, y) in zip(branches(t1), branches(t2))] + [x for x in branches(t1)[len(branches(t2)):]])
